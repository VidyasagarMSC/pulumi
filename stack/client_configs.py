...
def _create_advanced_openvpn_config(hostname, ca_cert, intermediate_cert, client_cert, client_key, enable_userpass=False):
    auth_lines = "auth-user-pass\nauth-retry interact\nauth-nocache\n" if enable_userpass else ""
...
def _create_simple_openvpn_config(hostname, ca_cert, intermediate_cert, client_cert, client_key, enable_userpass=False):
...
def _create_rootca_only_config(hostname, ca_cert, client_cert, client_key, enable_userpass=False):
    auth_lines = "auth-user-pass\nauth-retry interact\nauth-nocache\n" if enable_userpass else ""
...
def create_client_configs(instance_id, client_group, vpn_server, pki, cert_secrets, cfg):
    ...
    ).apply(lambda args: _create_advanced_openvpn_config(*args, enable_userpass=cfg.enable_userpass_auth))
    ...
    simple_client_config = pulumi.Output.all(
        ...
        pki["client_cert"].cert_pem,
        pki["client_private_key"].private_key_pem,
    ).apply(lambda args: _create_simple_openvpn_config(*args, enable_userpass=cfg.enable_userpass_auth))
    ...
    rootca_only_config = pulumi.Output.all(
        ...
        pki["client_cert"].cert_pem,
        pki["client_private_key"].private_key_pem,
    ).apply(lambda args: _create_rootca_only_config(*args, enable_userpass=cfg.enable_userpass_auth))
